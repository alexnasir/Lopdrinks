...
Updates recipe image URLs based on description pattern matching.
...
# Mapping (profile, inspiration, flavor) → Unsplash image URL
RECIPE_IMAGE_URLS: dict[tuple[str, str, str], str] = {
...
_FALLBACK_URL = "https://images.unsplash.com/photo-1512568400610-62da28bc8a13?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3"
_DESCRIPTION_RE = re.compile(
    r"A (\w+) coffee inspired by Kenya's ([\w\s]+)\. Features notes of (\w+)\."
)


def update_recipe_image_urls() -> None:
    """Update image URLs for all recipes based on their description."""
    ...
    with flask_app.app_context():
        ...
        for recipe in recipes:
            if not recipe.description:
                continue
            match = _DESCRIPTION_RE.match(recipe.description)
            if not match:
                continue
            profile, inspiration, flavor = (g.lower() for g in match.groups())
            new_url = RECIPE_IMAGE_URLS.get((profile, inspiration, flavor), _FALLBACK_URL)
