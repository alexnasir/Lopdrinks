...
/** Guarantee state is always a plain array regardless of what the service returns. */
...
const useRecipes = () => {
  ...
  useEffect(() => {
    ...
    recipeService
      .getAll()
      .then((data)  => { if (active) setRecipes(safeArray(data)); })
  ...
  const deleteRecipe = useCallback(async (id) => {
    ...
    setRecipes((prev) => safeArray(prev).filter((r) => r.id !== id));
